...
export class KarlsenWalletMobile extends KarlsenWalletUI {
    ...
    static get styles() {
        return [KarlsenWalletUI.styles, paginationStyle, swipeableStyle, css`
          ...
          .logo {
            //width: 30px;
            height: 30px; /*background-color:#DDD*/
          ...
          .info-ux table tr td:nth-child(2) {
            min-width: 150px;
          }

          .wallet-ux, .faucet-ux, .network-ux, .info-ux {
            margin: 24px 15px;
          }
          ...
          fa-icon.offline-icon {
            --fa-icon-size: 24px;
            --fa-icon-color: #aa0000;
          ...
          .theme-btn, .language-icon, .lock-btn {
            cursor: pointer
          }
            ...
            <div class="tabs-container hide-scrollbar" ?not-ready=${!isReady}>
                <flow-menu class="tabs" selected="${selectedTab}"
                    ...
                    ${this.showBalanceTab ? html`<a class="tab"
                                                    tab="balance" href="javascript:void 0"
                                                    is="i18n-a">Balance</a>` : ''}
            ...
            <div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
                <div class="flow-swipeable-row">
                    ...
                    <div class="tab-content ${sCls('wallet')}" for="wallet">
                        <div class="wallet-ux">
                            <div class="badge" is="i18n-div">RASPA WALLET</div>
                            ${PWAVersion ? html`
                                <div class="badge"><span is="i18n-span">Version</span> ${PWAVersion}</div>` : ''}
                            <div class="badge"><span is="i18n-span">Status:</span> ${this.status}</div>
                            <div class="badge"><span is="i18n-span">Network:</span>
                                ${(this.receiveAddress || "").split(":")[0] || ""}
                            </div>

                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      @click="${this.compoundUTXOs}" i18n>Compound Transactions
                            </flow-btn>
                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      @click="${this.exportTransactions}" i18n>Export transactions as CSV
                            </flow-btn>
                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      ?disabled=${this.updatingTransactionsTime}
                                      @click="${this.updateTransactionsTime}" i18n>Update transaction times
                            </flow-btn>
                            ${
                            ...
                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      @click="${this.showSeeds}" i18n>Backup Seed
                            </flow-btn>
                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      @click="${this.showRecoverWallet}" i18n>Recover From Seed
                            </flow-btn>
                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      @click="${this.exportWalletFile}" i18n>Export Wallet Seed File (KPK)
                            </flow-btn>
                            <flow-btn class="center-btn primary v-margin secondary default-button"
                                      @click="${this.importWalletFile}" i18n>Import Wallet Seed File (KPK)
                            </flow-btn>
                                            ...
                                            return html`
                                                <div class="donation-address-box badge">
                                                    ...
                                                    <fa-icon @click="${this.copyDonationAddress}"
                                                             icon="copy"
                                                             title="${i18n.t("Copy to clipboard")}"></fa-icon>
                                                </div>`
                                        })
                                }
                            </flow-expandable>
                                ...
                                <div class="faucet-ux">
                                    ...
                                    ${this.faucetPeriod ? html`
                                        <div class="margin-bottom">
                                            ${i18nHTMLFormat('Additional funds will be<br/>available in [n]', FlowFormat.duration(this.faucetPeriod))}
                                        </div>
                                    ` : ``}
                                    ${!this.faucetFundsAvailable ? html`` : html`
                                        <flow-btn class="primary" @click="${this.requestFaucetFunds}" i18n>Request Funds
                                            from Faucet
                                        </flow-btn>
                                    `}
                                </div>
                    ...
                    ${this.hideNetwork ? '' : html`
                        <div class="tab-content ${sCls('network')}" for="network">
                            <div class='network-ux'>
                                ...
                                ${!this.networkName ? html`
                                    <div>OFFLINE</div>` : html`
                                    <div>
                                        <table>
                                            <tr>
                                                <td>Network</td>
                                                <td>${this.networkName}</td>
                                            </tr>
                                            <tr>
                                                <td>DAA Score</td>
                                                <td>${FlowFormat.commas(this.blueScore)}</td>
                                            </tr>
                                            <tr>
                                                <td>DAG Header</td>
                                                <td>${FlowFormat.commas(this.headerCount)}</td>
                                            </tr>
                                            <tr>
                                                <td>DAG Blocks</td>
                                                <td>${FlowFormat.commas(this.blockCount)}</td>
                                            </tr>
                                            <tr>
                                                <td>Difficulty</td>
                                                <td>${FlowFormat.commas(this.difficulty, 2)}</td>
                                            </tr>
                                            <tr>
                                                <td>Median Offset</td>
                                                <td>${this.getTimeDelta(this.pastMedianTimeDiff)}</td>
                                            </tr>
                                            <tr>
                                                <td>Median Time UTC</td>
                                                <td>
                                                    ${this.pastMedianTime ? (new Date(this.pastMedianTime)).toJSON().replace(/T/, ' ').replace(/\..+$/, '') : ''}
                                                </td>
                                            </tr>
                                        </table>
                                    </div>
                    ...
                    ${this.hideDebug ? '' : html`
                        <div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
                            <div class="info-ux">
                                ...
                                <table>
                                    <tr>
                                        <td is="i18n-td">COUNT</td>
                                        <td>${inUseUTXOs.count}</td>
                                    </tr>
                                    <tr>
                                        <td is="i18n-td">AMOUNT</td>
                                        <td>${KLS(inUseUTXOs.satoshis || 0)} KLS</td>
                                    </tr>
                                </table>
                                <flow-btn class="center-btn primary v-margin secondary default-button"
                                          @click="${this.showUTXOs}" i18n>Show UTXOs
                                </flow-btn>
                                <flow-btn class="center-btn primary v-margin secondary default-button"
                                          @click="${this.forceTxTimeUpdate}" i18n>Force transaction times update
                                </flow-btn>
                                <flow-btn class="center-btn primary v-margin secondary default-button"
                                          @click="${this.scanMoreAddresses}" i18n>Scan More Addresses
                                </flow-btn>
            ...
            <div class="header" ?not-ready=${!isReady}>
                <div class="logo">
                    <img class="logo-img" @click=${this.toggleFullScreen}
                         src="${baseUrl + `/resources/images/${theme === "light" ? 'logo-light.png' : 'logo.png'}`}"/>
                </div>
                <div class="flex"></div>
                <div class='header-status' ?hidden=${!this.isOfflineBadge}>
                    <div class="header-row">
                        ...
                        <div>
                            <fa-icon class="offline-icon" icon="exclamation-triangle"></fa-icon>
                        </div>
                    </div>
                </div>
                ...
                <fa-icon class="lock-btn" ?hidden=${(this.locked || !this.wallet)}
                         ...
                         icon="lock"></fa-icon>
                ${this.hideI18nIcon ? '' : html`
                    <div class="language-icon">
                        <fa-icon icon="icons:language"
                                 @click="${this.onLangClick}"></fa-icon>
                    </div>`}
    ...
    renderLockScreen() {
        ...
        return html`
            <div class="lock-screen">
                <div><img class="big-logo" src="${baseUrl + `/resources/images/${theme === "light" ? 'logo-light.png' : 'logo.png'}`}"></div>
        ...
        return html`
            <flow-dropdown class="icon-trigger top-menu right-align">
                ...
                <flow-menu @click="${this.onMenuClick}" selector="_">
                    <flow-menu-item data-action="showSeeds">
                        <flow-i18n>Get Recovery Seed</flow-i18n>
                    </flow-menu-item>
                    <flow-menu-item data-action="showRecoverWallet">
                        <flow-i18n>Recover Wallet From Seed</flow-i18n>
                    </flow-menu-item>
            ...
            <div class="balance-badge">
                ${this.isLoading ? html`
                    ...
                    <div class="balance pending">
                        <span class="label-pending">${T('PLEASE WAIT')} <span
                                class="dots">${this.dots}</span> ${total ? this.formatKLS(total) + ' KLS' : ''}</span>
                    </div>
                ` : html`
                    <div class="balance">
