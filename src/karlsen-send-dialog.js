...
import {askForPassword, css, formatForMachine, getLocalWallet, html, KarlsenDialog, KLS} from './karlsen-dialog.js';
...
class KarlsenSendDialog extends KarlsenDialog {
    ...
    renderBody() {
        return html`
            ...
            <flow-checkbox class="calculate-network-fee" checked
                           @changed="${this.onCalculateFeeChange}">${T('Automatically calculate network fee')}
            </flow-checkbox>
            <flow-checkbox class="inclusive-fee"
                           @changed="${this.onInclusiveFeeChange}">${T('Include fee in the amount')}
            </flow-checkbox>
            ${this.renderEstimate()}
    ...
    renderEstimate() {
        if (this.estimateError)
            return html`
                <div class="estimate-tx-error">${this.estimateError}</div>`;
        ...
        return html`
            <div class="estimate-tx">
                <table>
                    ${txSize ? html`
                        <tr>
                            <td is="i18n-td">Transaction Size</td>
                            <td>${txSize.toFileSize()}</td>
                        </tr>` : ''}
                    ${dataFee ? html`
                        <tr>
                            <td is="i18n-td">Data Fee</td>
                            <td>${KLS(dataFee)} KLS</td>
                        </tr>` : ''}
                    ${fee ? html`
                        <tr>
                            <td is="i18n-td">Total Fee</td>
                            <td>${KLS(fee)} KLS</td>
                        </tr>` : ''}
                    ${totalAmount ? html`
                        <tr is="i18n-td">
                            <td>Total Amount</td>
                            <td> ${KLS(totalAmount)} KLS</td>
                        </tr>` : ''}
                </table>
            </div>
        `
    ...
    renderButtons() {
        ...
        return html`
            ...
            <flow-btn class="primary no-bg-button" @click="${this.cancel}" i18n>Cancel</flow-btn>
            <flow-btn class="secondary default-button" primary
