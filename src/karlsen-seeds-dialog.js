import {chunks, css, getRandomInt, html, i18n, KarlsenDialog, shuffle, T} from './karlsen-dialog.js';
...
class KarlsenSeedsDialog extends KarlsenDialog {
    ...
    static get styles() {
        return [KarlsenDialog.styles, css`
          ...
          .container {
            max-height: var(--karlsen-dialog-container-max-height, 660px);
            border-radius: 8px;
          }
          ...
          .button-row flow-btn {
            margin: 10px;
            flex: 1;
            --flow-btn-display: inline-block
          }

          .dot[icon="check"] {
            --fa-icon-color: var(--flow-primary-color)
          }
            ...
            <div class="dots">
                ${this.indexes.map((v, index) => {
                    ...
                    if (index < this.varificationStep)
                        ...
                        icon = "circle";
                    return html`
                        <fa-icon class="dot" icon="${icon}"></fa-icon>`
                })}
            </div>
    ...
    renderStep1Buttons() {
        if (this.args?.showOnlySeed)
            return
        return html`
            <flow-btn class="secondary default-button" primary @click="${e => this.step = 2}" i18n>NEXT</flow-btn>`
    }

    renderStep2Buttons() {
        return html`
            <flow-btn class="secondary default-button" primary @click="${e => this.step = 1}" i18n>BACK TO THE WORDS
            </flow-btn>`
    }

    renderStep3Buttons() {
        return html`
            <flow-btn class="secondary default-button" primary @click="${this.finish}" i18n>DONE</flow-btn>`
    }
